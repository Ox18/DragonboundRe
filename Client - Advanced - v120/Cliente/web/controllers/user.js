                                        ...
                                        conn.query('SELECT up.post_id, up.user_de, up.user_para, up.texto, up.fecha, u.game_id, u.rank, u.IdAcc FROM user_post up INNER JOIN users u ON u.IdAcc = up.user_de WHERE up.user_para = ? ORDER BY up.post_id DESC limit 0, 19', [rows_x4[0].user_para])
                                            ...
                                            if (rows5[0].length > 0) {
                                                ...
                                                for (i = 0; i < rows_x5.length; i++) {
                                                    ...
                                                    texto_test += '<input type="hidden" name="PostID" value="'+rows_x5[i].post_id+'">';
                                                    texto_test += '<div class="boxTitle ">';
                                                    texto_test += '<span class="DragonParse PostTitleEmo">(rank'+rows_x5[i].rank+')</span>';
                                                    texto_test += '<a class="name" href="/user/'+rows_x5[i].game_id+'">'+rows_x5[i].game_id+'</a>';
                                                    texto_test += '<span class="PostTime">';
                                                    texto_test += '<script>';
                                                    texto_test += 'TD('+rows_x5[i].fecha+')';
                                                    texto_test += '</script>';
                                                    texto_test += '</span>';
                                                    if (rows[0].IdAcc === req.session.account_id || rows_x5[i].IdAcc === req.session.account_id)
                                                        texto_test += '<input type="submit" class="DeletePost glow_button" name="action" value="delete">';
                                                    texto_test += '</div>';
                                                    texto_test += '<div class="boxBody boxBodyPost DragonParse">'+rows_x5[i].texto+'</div>';
                                                    texto_test += '</form>';
                                                    texto_test += '</div>';
                                                }
                                                res.render('info', {
